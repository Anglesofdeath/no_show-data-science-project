...
class DataProcessing:
    ...
    def clean_data(self):
        ...
        self.df['price'] = self.df.apply(lambda x: convertCurrencyStringToInt64(x['price']), axis=1)
        self.logger.info("price converted")
        self.logger.info(f"Size of DataFrame: {self.df.shape[0]}")
        self.df['room'] = self.df.apply(lambda x: fillRoomUsingPrice(x['room'], x['branch'], x['price']), axis=1)
        self.logger.info("room na filled")
        self.logger.info(f"Size of DataFrame: {self.df.shape[0]}")
        self.df['arrival_month'] = self.df['arrival_month'].apply(stringConvert)
        self.logger.info("arrival_month formatted")
        self.logger.info(f"Size of DataFrame: {self.df.shape[0]}")
        self.df['booked_months_before'] = self.df.apply(lambda x: bookedMonthsBefore(x['booking_month'], x['arrival_month']), axis=1)
